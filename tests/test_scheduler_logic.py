...
def test_followup_logic_query():
    ...
    with get_db_connection() as conn:
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?)
        ''', ('Followup Test', 'followup@test.com', 'Sent', (datetime.now(timezone.utc) - timedelta(days=3)).strftime('%Y-%m-%d %H:%M:%S'), 0))
        
        # Insert a mock lead that should NOT receive a followup yet
        cursor.execute('''
            INSERT INTO leads (name, email, status, email_sent_timestamp, followup_count)
            VALUES (?, ?, ?, ?, ?)
        ''', ('Too Soon Test', 'toosoon@test.com', 'Sent', (datetime.now(timezone.utc) - timedelta(hours=10)).strftime('%Y-%m-%d %H:%M:%S'), 0))
        ...
        for lead in leads:
            ...
            diff_hours = (datetime.now(timezone.utc) - last_action.replace(tzinfo=timezone.utc)).total_seconds() / 3600
